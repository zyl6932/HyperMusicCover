...
## 功能页改版方案（2026-09-09 规划，**还没动手**）

目标：**边看边调**——功能页顶上放一个手机形状的实时预览，下面是控件，改封面位置/时钟大小
当场就能看见。参照 KSU 的 `screen/colorpalette/ColorPaletteScreenMiuix.kt`。

### KSU 的做法（抄结构，不抄尺寸）

```
LazyColumn(horizontal 12dp)
  Spacer(32dp)
  ThemePreviewCard   ← fillMaxWidth(0.4f) + aspectRatio(LocalConfiguration 的真实屏幕比)
                       RoundedCornerShape(20dp) + 1dp outline，居中
  Spacer(72dp)
  TabRow(三个主题模式)     ← top.yukonga.miuix.kmp.basic.TabRow
  Card { 开关 + AnimatedVisibility { 更多 } }
```

预览是**用 Box 拼出来的活物**（假顶栏/假卡片/假导航栏，全部吃当前配色），不是截图；
变矮时用 `BoxWithConstraints` 少画几张小卡片。小集合的互斥选项用 `TabRow` 不用下拉。

### 我们的结构（跟 KSU 有两处不同，都是有理由的）

```
Column {
    预览          ← 钉住，不进滚动区
    TabRow: 封面 │ 时钟 │ 媒体卡片
    LazyColumn { 当前这组的控件 }    ← 只有这里滚
}
```

- **预览必须移出滚动区**。KSU 把预览当 `LazyColumn` 的一个 `item`，往下滚就没了；
  他们能忍是因为控件短。我们的预览就是"正在调的东西本身"，滚掉就废了。
- **TabRow 只是就地换卡片内容，不导航**。所以预览一份就够，不需要在卡片里再嵌一层。

**竖向空间很紧，必须算**（2608px / density 3.0 = 869dp，扣状态栏+顶栏 52+底部导航后约 690dp）：

| | 照抄 KSU | 建议 |
|---|---|---|
| 预览宽度 | 0.42 → 173×375dp | **0.38 → 157×341dp** |
| 上/下留白 | 32 / 72 | **24 / 20** |
| 剩给控件 | ~200dp ≈ 3 行滑块 | **~260dp ≈ 4 行** |

KSU 那个 72dp 的下留白我们负担不起。一组控件真放不下时，逃生口是
**让预览跟着滚动收缩**（现成的 `MiuixScrollBehavior` 能驱动），**不是再复制一个预览**。

### 一屏一个活预览

- 一屏之内换控件 → TabRow 就地切，预览钉在上面，**一份**
- 值得独立成页的（比如时钟取色做成色轮） → **那一页自带一份预览**。
  KSU 就是这么干的：`ColorPaletteScreenMiuix` 有自己的 `Scaffold` 和顶栏，预览长在它自己里面。
- **绝不在同一屏放两个活预览**：两份要同步的状态、两倍合成开销，没有收益。

### 三个必须先定的技术问题

**① 封面图从哪来。** app 拿不到 MediaSession（要通知使用权），只能问模块要。
方案：探针加 `op=art`，模块在**有序广播的 `setResultExtras` 里回一张 256px 的 JPEG**
（整屏那张才 80KB，256px 缩略图 10~20KB，离 binder 的 1MB 很远）。

**② 合成在哪做。** **在 app 里本地合成**——拖滑块时每帧往 SystemUI 打来回必然卡。
代价是**镜像+模糊+bias 那套排版会有两份实现，会漂**。缓解：只搬布局数学（模糊在预览里可以糊弄，
那么小看不出来），并且明确**模块是真相、预览是示意**。

**③ 时钟画不准，这是硬限制。** OEM 的 `all_in_one` 可变字体在 SystemUI / `com.miui.clock` 里，
app 拿不到。预览里的时钟只能是**替身**：位置和尺寸准（这两个正是滑块在调的），字形不准。
字重/字宽将来要调的话，替身只能用系统可变字体近似。**UI 上要说清楚，别让人以为所见即所得。**

（媒体卡片的真实位置也没量过，先硬编码近似矩形，或者加个 `op=bounds` 那样的探针量一次。）

### 三组控件

| 组 | 现有 | 要补的 |
|---|---|---|
| 封面 | 纵向位置 `bias` | 排版样式（全屏封面 / 专辑卡片，状态卡第三行已留位） |
| 时钟 | 大小、玻璃强度 | 字重/字宽（模块已有 `op=params`）、取色（待办第 1 条） |
| 媒体卡片 | 无 | 模块目前完全没碰它，全新一块 |

### 实施顺序

1. **`op=art` + 预览控件（只画封面层）** —— 收益最大、解锁其余，且不碰模块渲染逻辑，风险最低
2. **把现有三个滑块搬到预览下面 + TabRow 分组**
3. **时钟替身层**
4. **新控件**，跟着模块侧能力一个个加

### 还没定的

- 时钟替身能不能接受？还是预览干脆不画时钟（不画就没有"调了字体没反应"的困惑）
- 媒体卡片那组到底想调什么？模块现在完全没碰它，得先知道目标才能定 hook 哪里
...
7. **支持另一种封面样式**（用户提过要加，UI 已经给它留了位置；见上面「功能页改版方案」）。现在 `composeWallpaper()`
