...
| 功能 | 跟随媒体卡片、立即应用/恢复、封面纵向位置、时钟缩放、玻璃填充、隐藏景深抠图、修复锁屏壁纸、重启系统界面 |
...
3. 重启系统界面。
4. **锁屏壁纸必须和桌面壁纸是两张不同的图**——共用一张时 MIUI 根本不会创建 keyguard 的壁纸
   渲染器，整套机制无处可挂。模块会自己检测并修复（把桌面壁纸复制一份设成锁屏壁纸，肉眼没有区别），
   也可以在「功能 → 修复锁屏壁纸」手动触发。
